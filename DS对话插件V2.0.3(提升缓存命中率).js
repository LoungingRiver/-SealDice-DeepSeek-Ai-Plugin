...
// @author       懒河
// @version      2.0.3
// @description  基于原本白鱼制作的插件的修改，完善了系统设定和对话存储，添加了指令功能，添加了Markdown过滤功能，使用.deepseekai查看相应指令。新增对话摘要功能优化记忆系统。新增Temperature配置功能。简化资料库存储。增强兼容性。优化资料库整合机制。
// @license      MIT
...
if (!seal.ext.find('deepseekai')) {
    const ext = seal.ext.new('deepseekai', 'lanhe', '2.0.3');
    seal.ext.register(ext);

    // 资料库内容版本追踪
    let currentLibraryHash = '';
    ...
    const configs = {
        ...
        "摘要生成最大tokens数": "1000",
        "存储上下文对话限制轮数": "16",
        "角色设定": "你是一只黑猫骰娘，主要用于测试，乐于为测试者提供一切需要的服务。在必要时应当回答包括已知的时间信息，摘要内容等相关的提问。",
        "非指令关键词": "小伊",
        "Temperature": "1.3",
    ...
    // 计算资料库内容的哈希值
    function calculateLibraryHash() {
        const libraries = ["full_library", "sub1_library", "sub2_library", "sub3_library"];
        let content = '';
        
        libraries.forEach(libName => {
            const libContent = seal.ext.getStringConfig(ext, libName) || '';
            content += libContent;
        });
    ...
    // 检查资料库是否发生变化
    function checkLibraryChanges() {
        const newHash = calculateLibraryHash();
        
        if (currentLibraryHash !== '' && newHash !== currentLibraryHash) {
            console.log(`[资料库变更检测] 检测到资料库内容变化，旧哈希: ${currentLibraryHash.substring(0, 8)}... 新哈希: ${newHash.substring(0, 8)}...`);
            updateAllUsersSystemContext();
        }
        
        currentLibraryHash = newHash;
    }

    // 更新所有用户的系统上下文
    function updateAllUsersSystemContext() {
        console.log("[资料库更新] 开始更新所有用户的系统上下文");
        ...
        try {
            ...
            for (const key in storage) {
                if (storage.hasOwnProperty(key)) {
                    if (!key.includes('_summary') && (key.length === 10 || key.length === 11) && /^\d+$/.test(key)) {
                        try {
                            const userId = key;
                            const ai = new DeepseekAI(userId);
                            ai.updateSystemContext(true); // 强制更新
                            updatedCount++;
                            
                            if (updatedCount % 10 === 0) {
                                console.log(`[资料库更新] 已更新 ${updatedCount} 个用户`);
                            }
                        } catch (error) {
                            console.error(`[资料库更新错误] 用户 ${key}:`, error);
                        }
                    }
                }
            }
            console.log(`[资料库更新完成] 总共更新了 ${updatedCount} 个用户`);
        } catch (error) {
            console.error("[资料库更新全局错误]:", error);
        }
    }
    class DeepseekAI {
        constructor(userId) {
        ...
        updateSystemContext(forceUpdate = false) {
            try {
                let systemContent = seal.ext.getStringConfig(ext, "角色设定");
                const summary = this.loadSummary();
                ...
                if (summary.content && summary.content.trim().length > 0) {
                    ...
                    console.log(`[系统上下文更新] 用户 ${this.userId} 系统提示词已包含摘要和资料库，长度: ${systemContent.length}`);
                } else {
                    console.log(`[系统上下文更新] 用户 ${this.userId} 系统提示词包含资料库但不包含摘要`);
                }
                ...
                this._ensureSystemMessage(systemContent);
                if (forceUpdate) {
    ...
    // 新增资料库更新指令
    const cmdUpdateLibrary = seal.ext.newCmdItemInfo();
    cmdUpdateLibrary.name = "更新资料库";
    cmdUpdateLibrary.help = "手动更新所有用户的资料库内容";
    cmdUpdateLibrary.solve = (ctx, msg) => {
        updateAllUsersSystemContext();
        seal.replyToSender(ctx, msg, "已触发资料库更新，所有用户的系统上下文将使用最新的资料库内容");
    };
    ...
    ext.cmdMap["更新资料库"] = cmdUpdateLibrary;
    ...
    cmdHelp.solve = (ctx, msg) => {
        ...
        helpMsg += "10. 更新资料库 - 手动更新所有用户的资料库内容\n\n";
        helpMsg += `当前状态:\n`;
    ...
    // 初始化时计算初始哈希
    setTimeout(() => {
        currentLibraryHash = calculateLibraryHash();
        console.log(`[资料库初始化] 初始资料库哈希: ${currentLibraryHash.substring(0, 8)}...`);
        console.log(`[Deepseek AI插件加载完成] 版本 2.0.3 (资料库优化版)`);
    }, 1000);
}
